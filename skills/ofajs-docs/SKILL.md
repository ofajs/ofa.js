...
| `ofaElement.scrollTop` | `ofaElement.ele.scrollTop` | ofa.js 对象通过 .ele 访问原生属性 |
| `ofaElement.scrollHeight` | `ofaElement.ele.scrollHeight` | ofa.js 对象通过 .ele 访问原生属性 |
...
### ofa.js 对象 vs 原生 DOM 元素

#### ofa.js 包装对象
- 通过 `$()` 方法获取的是 **ofa.js 包装对象**
- 提供了 ofa.js 的增强方法和响应式特性
- 例如：`this.shadow.$("#messages")`（`this` 仅在模板方法或生命周期中可用）

#### 原生 DOM 元素
- 通过 `.ele` 属性访问原生 DOM 元素
- 用于操作原生 DOM 属性和方法
- 例如：`messagesDiv.ele.scrollTop`

#### shadow 对象的选择器方法
在 ofa.js 中：
- `this.shadow` 返回的是 **ofa.js 实例化的对象**，不是原生的 ShadowRoot
- 应该使用 `$()` 方法选择元素，而不是原生的 `getElementById()` 或 `querySelector()`
...
#### ofa.js 对象与原生 DOM 元素的访问
- `element.$()` 返回 **ofa.js 包装对象**
- `.ele` 属性获取 **原生 DOM 元素**
- 原生 DOM 属性和方法需要通过 `.ele` 访问
...
#### 使用场景
- **ofa.js 方法**：使用 ofa.js 对象的方法（如 `.on()`, `.addClass()`, `.text` 等）
...
- **样式状态切换**（如 active、disabled、visible）→ 使用 `class:` + 类名选择器
