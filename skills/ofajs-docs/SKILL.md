...
**重要提醒**：
- `<script>` 必须放在 `<template>` 内部，不能放在外部
- `<style>`、模板内容和 `<script>` 都在同一个 `<template>` 标签内
- 这是 ofa.js 的标准结构，与 Vue/React 的单文件组件结构不同
...
| ❌ 错误写法 | ✅ 正确写法 | 说明 |
|------------|-----------|------|
| `computed: { double() {} }` | `proto: { get double() {} }` | ofa.js 使用 getter 定义计算属性，需放在 proto 中 |
...
|  `.hide()` `.show()`| `.style.display = "none"` `.style.display = ""` | ofa.js 没有 jQuery 风格的 show/hide 方法 |
| `.html("xxx")` `.text("xxx")` | `.html = "xxx"` `.text = "xxx"` | 直接设置属性而非调用方法 |
| `<script>` 在 `<template>` 外部 | `<script>` 在 `<template>` 内部 | ofa.js 的 script 必须放在 template 标签内部 |
...
| `:class="$data.type"` | `attr:type="$data.type"` + `.message[type="sent"]` | 数据固有属性应使用 attr: 绑定，配合属性选择器 |
...
├─ 是 → 使用 o-fill 组件
│   └─ 选择渲染方式：
│       ├─ 直接渲染（推荐用于简单结构）→ 模板内容直接写在 o-fill 内部，不需要 <template> 包裹
│       └─ 模板渲染（用于复杂结构或复用）→ <template> 定义在 o-fill 外部，使用 name 属性绑定
└─ 否 → 正常编写模板
