...
> **重要说明**：
> - `goto`、`replace`、`back` メソッドは、**ページインスタンス（page）** と**アプリケーションインスタンス（app）** の両方で使用できます
> - `forward` メソッドは**アプリケーションインスタンス（app）** でのみ使用できます
> - ページ内で使用する場合：`this.goto()` または `this.app.goto()`
> - コンポーネント内で使用する場合：`this.app.goto()`（app インスタンスを通じて呼び出す必要があります）
> 
> **パスの相対性に関する説明**：
> - **ページモジュール**上で `goto` または `replace` を使用する場合、パスは**現在のページモジュール**のアドレスを基準とします
> - **アプリケーションインスタンス（app）** 上で `goto` または `replace` を使用する場合、パスは**現在のアプリケーションのナビゲーション**を基準とします
